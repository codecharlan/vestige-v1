...
class GraveyardProvider { [key: string]: any;
    ...
    async refresh(repoPath) {
        ...
        const files = await this.gitAnalyzer.findDeletedFiles(repoPath);
        // Commands downstream (view/resurrect) need to know which repo the
        // deleted file belonged to.
        this.deletedFiles = files.map(f => ({ ...f, repoPath }));
...
class GraveyardPanel { [key: string]: any;
